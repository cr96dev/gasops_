...
try {
  const res = await fetch('/api/analyze', {
    ...
    body: JSON.stringify({
      prompt: `Analiza este reporte de Wetstock Management de una estación de combustible en Guatemala.
      Identifica: 1) Combustibles en alerta y su riesgo, 2) Tendencias de pérdida o ganancia,
      3) Días consecutivos de pérdida preocupantes, 4) Recomendaciones específicas de acción inmediata.
      Sé conciso y directo. Usa el contexto de que los límites regulatorios son los indicados.`,
      data: {
        estacion: 'SS Oakland',
        fecha: new Date().toLocaleDateString('es-GT'),
        alertas_activas: alertas,
        combustibles: resumen
      }
    })
  })
  ...
  setAnalisisIA(`Error al analizar: ${err.message}`)
  setModalIA(true)
}
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setModalIA(false)}>
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white">
          <h2 className="text-base font-semibold text-gray-900">🤖 Análisis IA — Wetstock</h2>
          <button onClick={() => setModalIA(false)} className="text-gray-400 hover:text-gray-600 text-xl">✕</button>
        </div>
        <div className="px-6 py-5">
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: analisisIA
                .replace(/##\s(.+)/g, "<h3 style='font-size:14px;font-weight:600;margin:16px 0 6px'>$1</h3>")
