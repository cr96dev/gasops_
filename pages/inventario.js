...
export default function Inventario({ session }) {
  ...
  const [inventarioConsolidado, setInventarioConsolidado] = useState([])
  const [loading, setLoading] = useState(true)
  const [tab, setTab] = useState('actual')
  const [vistaAdmin, setVistaAdmin] = useState('consolidada')
  ...
  async function loadData() {
    ...
    if (p?.rol === 'admin') {
      ...
      const { data: invAll } = await supabase
        .from('inventario')
        .select('*, estaciones(id, nombre)')
        .order('producto')
      setInventarioConsolidado(invAll || [])
  ...
  async function guardarEntrega(e) {
    ...
    const estacionDestino = perfil?.rol === 'admin' ? estacionAdmin : perfil?.estacion_id
    if (!estacionDestino) {
      setErrorEntrega('Selecciona una estación de destino antes de registrar la entrega.')
      return
    }
    ...
    for (const item of itemsEntrega) {
      ...
      const { data: inv } = await supabase.from('inventario').select('id, stock_actual')
        .eq('estacion_id', estacionDestino).ilike('producto', item.nombre).single()
  ...
  function descargarPlantillaInicial() {
    const filas = PRODUCTOS_CATALOGO.map(p => ({
      ...
      'Stock actual': 0, 'Stock minimo': 2, Unidad: p.unidad,
    }))
  ...
  async function guardarCargaInicial() {
    ...
    const estacionDestino = perfil?.rol === 'admin' ? estacionAdmin : perfil?.estacion_id
    if (!estacionDestino) {
      setErrorInicial('Selecciona una estación de destino antes de guardar.')
      return
    }
    ...
    for (const item of previaInicial) {
      const { data: existe } = await supabase.from('inventario').select('id')
        .eq('estacion_id', estacionDestino).ilike('producto', item.producto).single()
      if (existe) {
        const { error } = await supabase.from('inventario').update({
        ...
        const { error } = await supabase.from('inventario').insert({
          estacion_id: estacionDestino, producto: item.producto,
  ...
  // ── Cálculos vista consolidada ──
  const productosUnicos = [...new Set(inventarioConsolidado.map(i => i.producto))].sort()
  const estacionesUnicas = [...new Map(
    inventarioConsolidado
      .filter(i => i.estaciones)
      .map(i => [i.estaciones.id, { id: i.estaciones.id, nombre: i.estaciones.nombre }])
  ).values()].sort((a, b) => a.nombre.localeCompare(b.nombre))

  const categoriasConsolidadas = ['Todas', ...new Set(inventarioConsolidado.map(i => i.categoria).filter(Boolean))]

  const productosConsolidados = productosUnicos
    .filter(prod => {
      const ej = inventarioConsolidado.find(i => i.producto === prod)
      const matchBusqueda = prod?.toLowerCase().includes(busqueda.toLowerCase())
      const matchCat = filtroCat === 'Todas' || ej?.categoria === filtroCat
      return matchBusqueda && matchCat
    })
    .map(prod => {
      const filas = inventarioConsolidado.filter(i => i.producto === prod)
      const porEstacion = {}
      let total = 0
      let bajos = 0
      for (const f of filas) {
        const stock = parseFloat(f.stock_actual) || 0
        const min = parseFloat(f.stock_minimo) || 0
        porEstacion[f.estaciones?.id] = { stock, min, bajo: stock <= min }
        total += stock
        if (stock <= min) bajos++
      }
      return {
  ...
  const totalBajoStockConsolidado = inventarioConsolidado.filter(
    i => parseFloat(i.stock_actual) <= parseFloat(i.stock_minimo)
  ).length

  const catsActivas = perfil?.rol === 'admin' && tab === 'actual' && vistaAdmin === 'consolidada'
      ...
      <div className="p-6 max-w-6xl">
        <div className="flex items-start justify-between mb-5">
          <div>
            ...
            <p className="text-sm text-gray-400">
              {perfil?.rol === 'admin'
                ? `${estacionesUnicas.length} estaciones · ${inventarioConsolidado.length} registros`
                : `${estacion?.nombre} · ${totalItems} productos`}
            </p>
          </div>
          {(perfil?.rol === 'admin' ? totalBajoStockConsolidado > 0 : bajoStock.length > 0) && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5 text-xs text-amber-700 font-medium">
              ⚠ {perfil?.rol === 'admin' ? totalBajoStockConsolidado : bajoStock.length} bajo mínimo
            </div>
          )}
        </div>
        ...
        {perfil?.rol === 'admin' && tab === 'actual' && (
          <div className="flex gap-2 mb-4">
            <button onClick={() => setVistaAdmin('consolidada')}
              className={`px-3 py-1.5 text-xs rounded-lg border transition-colors ${vistaAdmin === 'consolidada' ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
              Vista consolidada
            </button>
            <button onClick={() => setVistaAdmin('estacion')}
              className={`px-3 py-1.5 text-xs rounded-lg border transition-colors ${vistaAdmin === 'estacion' ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
              Por estación
            </button>
          </div>
        )}

        {/* Selector de estación cuando aplica */}
        {perfil?.rol === 'admin' && (
          (tab === 'actual' && vistaAdmin === 'estacion') ||
          tab === 'entrega' ||
          tab === 'inicial'
        ) && (
          <div className="mb-4">
            <label className="text-xs text-gray-500 block mb-1">
              {tab === 'actual' ? 'Ver inventario de:' : 'Estación de destino:'}
            </label>
        ...
        {/* ── Tab: Stock actual — VISTA CONSOLIDADA ── */}
        {tab === 'actual' && perfil?.rol === 'admin' && vistaAdmin === 'consolidada' && (
          <>
            <div className="flex gap-3 mb-4">
              ...
              <select value={filtroCat} onChange={e => setFiltroCat(e.target.value)}
                ...
                {catsActivas.map(c => <option key={c}>{c}</option>)}
              </select>
            </div>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-gray-100 bg-gray-50">
                      <th className="px-3 py-2.5 text-left text-gray-500 font-medium sticky left-0 bg-gray-50 min-w-[200px]">Producto</th>
                      {estacionesUnicas.map(e => (
                        <th key={e.id} className="px-2 py-2.5 text-center text-gray-500 font-medium whitespace-nowrap">
                          {e.nombre.replace('SS ', '')}
                        </th>
                      ))}
                      <th className="px-3 py-2.5 text-center text-gray-700 font-semibold bg-blue-50">Total</th>
                    </tr>
                  </thead>
                  <tbody>
                    {productosConsolidados.map((p, i) => (
                      <tr key={i} className="border-b border-gray-50 hover:bg-gray-50">
                        <td className="px-3 py-2 sticky left-0 bg-white font-medium text-gray-800">
                          <div>{p.producto}</div>
                          <div className="text-xs text-gray-400 font-normal">{p.categoria} · {p.unidad}</div>
                        </td>
                        {estacionesUnicas.map(e => {
                          const dato = p.porEstacion[e.id]
                          if (!dato) return <td key={e.id} className="px-2 py-2 text-center text-gray-300">—</td>
                          return (
                            <td key={e.id} className={`px-2 py-2 text-center ${dato.bajo ? 'bg-amber-50 text-amber-700 font-medium' : 'text-gray-700'}`}>
                              {dato.stock}
                            </td>
                          )
                        })}
                        <td className={`px-3 py-2 text-center font-semibold ${p.bajos > 0 ? 'bg-amber-50 text-amber-800' : 'bg-blue-50 text-blue-900'}`}>
                          {p.total}
                        </td>
                      </tr>
                    ))}
            ...
            <p className="text-xs text-gray-400 mt-3">
              Las celdas en amarillo indican stock bajo el mínimo. Cambia a "Por estación" para editar.
            </p>
          </>
        )}

        {/* ── Tab: Stock actual — VISTA POR ESTACIÓN o GERENTE ── */}
        {tab === 'actual' && (perfil?.rol !== 'admin' || vistaAdmin === 'estacion') && (
          <>
            {(perfil?.rol === 'admin' && !estacionAdmin) ? (
              <div className="bg-white rounded-xl border border-gray-100 p-10 text-center text-sm text-gray-400">
                Selecciona una estación para ver su inventario
              </div>
            ) : inventario.length === 0 ? (
