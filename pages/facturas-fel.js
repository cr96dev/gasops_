...
// ─── Credenciales INFILE ──────────────────────────────────────────────────────
const SIGNER_URL     = 'https://signer-emisores.feel.com.gt/sign_solicitud_firmas/firma_xml'
const CERT_URL       = 'https://certificador.feel.com.gt/fel/certificacion/v2/dte/'
...
function generarXMLFactura({ estacionId, receptor, items, fecha }) {
  ...
  const iva = (monto) => {
    const gravable = parseFloat((monto / 1.12).toFixed(10))
    const impuesto = parseFloat((monto - gravable).toFixed(10))
    return { gravable, impuesto }
  }

  const itemsXml = items.map((item, i) => {
    const total = parseFloat((item.cantidad * item.precio).toFixed(2))
    const { gravable, impuesto } = iva(total)
    return `
  ...
  const granTotal = items.reduce((s, i) => s + i.cantidad * i.precio, 0)
  const totalIva  = iva(granTotal).impuesto
...
async function firmarXML(xml, codigoInterno) {
  ...
  // El signer devuelve el XML firmado directamente (no JSON)
  if (!text || text.includes('error') || text.includes('Error')) {
    throw new Error('Error en firma: ' + text.substring(0, 200))
  }
  return text // XML firmado en texto
}
...
async function certificarXML(xmlFirmado, codigoInterno) {
  ...
  if (!res.ok) throw new Error(`Certificador HTTP ${res.status}`)
...
export default function FacturasFEL({ session }) {
  ...
  // Totales FEL sin límite de registros
  ...
  const getFechasVista = useCallback((vista) => {
    const hoy = new Date()
    const fmt = d => d.toISOString().split('T')[0]
    if (vista === 'hoy')  return { ini: fmt(hoy), fin: fmt(hoy) }
    if (vista === 'ayer') { const a = new Date(hoy); a.setDate(hoy.getDate()-1); return { ini: fmt(a), fin: fmt(a) } }
    ...
    return { ini: filtros.fechaInicio, fin: filtros.fechaFin }
  }, [filtros.fechaInicio, filtros.fechaFin])

  const buscar = useCallback(async (f = filtros) => {
    setCargando(true)
    const { ini, fin } = getFechasVista(f.vista)
    const fi = f.vista === 'personalizado' ? f.fechaInicio : ini
    const ff = f.vista === 'personalizado' ? f.fechaFin    : fin

    // Totales sin límite (consulta separada)
    let qtotal = supabase.from('facturas_fel_items')
      .select('total', { count: 'exact' })
      .gte('fecha', fi).lte('fecha', ff)
    if (f.estacion !== 'todas') qtotal = qtotal.eq('estacion_id', f.estacion)

    // Suma total
    let qmonto = supabase.from('facturas').select('monto')
      .eq('sincronizado_infile', true)
      .gte('fecha_emision', fi).lte('fecha_emision', ff)
    if (f.estacion !== 'todas') qmonto = qmonto.eq('estacion_id', f.estacion)

    const [{ count }, { data: montos }] = await Promise.all([qtotal, qmonto])
    const totalMonto = (montos || []).reduce((s, r) => s + parseFloat(r.monto || 0), 0)
    setTotalesFEL({ cantidad: montos?.length || 0, monto: totalMonto })

    // Resumen por estación y fecha
    const { data: resumen } = await supabase.from('facturas')
      .select('estacion_id, fecha_emision, monto, estaciones(nombre)')
      .eq('sincronizado_infile', true)
      .gte('fecha_emision', fi).lte('fecha_emision', ff)
    ...
    if (f.estacion !== 'todas') query = query.eq('estacion_id', f.estacion)
    const { data } = await query
    setFacturas(data || [])
    setCargando(false)
  }, [filtros, getFechasVista])

  useEffect(() => { if (!loading) buscar() }, [loading])

  function cambiarVista(vista) {
    const nuevos = { ...filtros, vista }
    setFiltros(nuevos)
    buscar(nuevos)
  }
  ...
  function agregarItem() {
    setItemsEmit(prev => [...prev, { descripcion: '', cantidad: 1, precio: '' }])
  }
  function quitarItem(i) {
    setItemsEmit(prev => prev.filter((_, idx) => idx !== i))
  }
  function actualizarItem(i, campo, valor) {
    setItemsEmit(prev => prev.map((it, idx) => idx === i ? { ...it, [campo]: valor } : it))
  }
  ...
  async function emitirFactura() {
    ...
    if (itemsEmit.some(i => !i.descripcion || !i.precio || parseFloat(i.precio) <= 0)) {
      toast('Completa todos los productos', 'warning'); return
    }
    ...
    try {
      ...
      if (!certResp.resultado) {
        const errores = (certResp.descripcion_errores || []).map(e => e.mensaje_error || e.validacion).join(' | ')
        throw new Error(errores || certResp.descripcion || 'Error en certificación')
      }

      // Éxito — guardar en tienda o facturas_fel según estación
      const uuid    = certResp.uuid
      const serie   = certResp.serie
      const numero  = certResp.numero
      const granTotal = totalEmision
      ...
      if (esOakland) {
        ...
        const { data: factInsertada } = await supabase.from('facturas').insert({
          ...
          fecha_emision:       fechaHora.split('T')[0],
          fecha_vencimiento:   fechaHora.split('T')[0],
          monto:               granTotal,
          estado:              'pagada',
  ...
  async function exportarExcel() {
    const { ini, fin } = getFechasVista(filtros.vista)
    const fi = filtros.vista === 'personalizado' ? filtros.fechaInicio : ini
    const ff = filtros.vista === 'personalizado' ? filtros.fechaFin    : fin

    const wb = XLSX.utils.book_new()

    // Hoja 1: Facturas
    let qf = supabase.from('facturas').select('*, estaciones(nombre)')
      .eq('sincronizado_infile', true).gte('fecha_emision', fi).lte('fecha_emision', ff).order('fecha_emision')
    ...
    const ws1 = XLSX.utils.json_to_sheet((facts||[]).map(f => ({
      Fecha: f.fecha_emision, Estación: f.estaciones?.nombre, 'No. Factura': f.numero_factura,
      Cliente: f.proveedor, 'Monto (Q)': parseFloat(f.monto), UUID: f.uuid_fel||''
    })))
    ...
    const ws2 = XLSX.utils.json_to_sheet(allItems.map(it => ({
      Fecha: it.fecha, Descripción: it.descripcion, Cantidad: it.cantidad,
      Unidad: it.unidad, 'Precio Unitario': it.precio_unitario, 'Total (Q)': it.total
    })))
    XLSX.utils.book_append_sheet(wb, ws2, 'Detalle ítems')

    // Hoja 3: Resumen por estación
    const ws3 = XLSX.utils.json_to_sheet(resumenEstaciones.map(r => ({
      Estación: r.nombre, 'Cantidad facturas': r.cantidad, 'Total (Q)': parseFloat(r.monto.toFixed(2))
    })))
    XLSX.utils.book_append_sheet(wb, ws3, 'Resumen estaciones')

    XLSX.writeFile(wb, `FacturasFEL_${fi}_al_${ff}.xlsx`)
      ...
      <div className="p-6 max-w-6xl">
        ...
        <div className="flex gap-1 mb-5 border-b border-gray-100">
          <button onClick={() => setTab('ver')}
            className={`px-4 py-2 text-sm border-b-2 transition-colors ${tab==='ver'?'border-blue-600 text-blue-700 font-medium':'border-transparent text-gray-500 hover:text-gray-700'}`}>
            Ver facturas
          </button>
          <button onClick={() => setTab('emitir')}
            className={`px-4 py-2 text-sm border-b-2 transition-colors ${tab==='emitir'?'border-blue-600 text-blue-700 font-medium':'border-transparent text-gray-500 hover:text-gray-700'}`}>
            + Emitir factura
          </button>
        </div>
          ...
          <>
            ...
            <div className="flex items-center gap-2 mb-4 flex-wrap">
              ...
              <div className="ml-auto flex gap-2 items-center">
                <select value={filtros.estacion} onChange={e => { const nf={...filtros, estacion:e.target.value}; setFiltros(nf); buscar(nf) }}
            ...
            {filtros.vista === 'personalizado' && (
              <div className="flex gap-3 mb-4">
                ...
                <button onClick={() => buscar()} className="mt-5 bg-blue-600 text-white text-sm px-4 py-1.5 rounded-lg hover:bg-blue-700">
                  Buscar
                </button>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              <div className="bg-blue-50 rounded-xl p-4">
                ...
                <div className="text-2xl font-medium text-blue-800">{totalesFEL.cantidad.toLocaleString('es-GT')}</div>
              </div>
              <div className="bg-blue-50 rounded-xl p-4">
                <div className="text-xs text-blue-600 mb-1">Monto total</div>
                <div className="text-2xl font-medium text-blue-800">Q{totalesFEL.monto.toLocaleString('es-GT',{maximumFractionDigits:2})}</div>
              </div>
              <div className="bg-blue-50 rounded-xl p-4">
                <div className="text-xs text-blue-600 mb-1">Promedio por factura</div>
                <div className="text-2xl font-medium text-blue-800">
                  {totalesFEL.cantidad>0?`Q${(totalesFEL.monto/totalesFEL.cantidad).toLocaleString('es-GT',{maximumFractionDigits:2})}`:'—'}
                </div>
              </div>
              <div className="bg-blue-50 rounded-xl p-4">
                <div className="text-xs text-blue-600 mb-1">Estaciones activas</div>
                <div className="text-2xl font-medium text-blue-800">{resumenEstaciones.length}</div>
              </div>
            </div>
                  ...
                  <tbody>
                    {resumenEstaciones.map((r, i) => (
                      <tr key={i} className="border-b border-gray-50 hover:bg-gray-50">
                        <td className="px-5 py-2.5 text-gray-800 font-medium text-xs">{r.nombre}</td>
            ...
            <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
                      ...
                      <>
                        <tr key={f.id} className="border-b border-gray-50 hover:bg-gray-50">
                          ...
                          <td className="px-4 py-2.5 text-center">
                            <button onClick={() => verItems(f.id)} className="text-xs text-blue-600 hover:text-blue-800">
              ...
              {facturasFiltradas.length > 0 && (
                <div className="px-5 py-2.5 border-t border-gray-100 text-xs text-gray-400">
                  Mostrando {facturasFiltradas.length} de {totalesFEL.cantidad} facturas — total real incluye todas las páginas
                </div>
              )}
            </div>
          </>
          ...
          <div className="max-w-2xl space-y-4">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-3 flex items-start gap-3">
              ...
              <p className="text-xs text-amber-700">
                <span className="font-medium">Modo demo</span> — Las facturas se emiten con credenciales de prueba (NIT 11700574K). La serie aparecerá como <code className="bg-amber-100 px-1 rounded">** PRUEBAS **</code> y no tiene validez fiscal ante la SAT. Cuando tengas credenciales de producción se actualiza con un solo cambio.
              </p>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-gray-500 block mb-1">NIT (o CF)</label>
                  <input value={receptor.nit} onChange={e => setReceptor(r => ({ ...r, nit: e.target.value }))}
                    placeholder="CF"
                ...
                <div>
                  ...
                  <input value={receptor.nombre} onChange={e => setReceptor(r => ({ ...r, nombre: e.target.value }))}
                    placeholder="Consumidor Final"
                    className="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-blue-400" />
                </div>
                <div>
                  <label className="text-xs text-gray-500 block mb-1">Correo (opcional)</label>
                  <input value={receptor.correo} onChange={e => setReceptor(r => ({ ...r, correo: e.target.value }))}
                    placeholder="cliente@email.com"
                    className="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-blue-400" />
                </div>
                <div>
                  <label className="text-xs text-gray-500 block mb-1">Dirección (opcional)</label>
                  <input value={receptor.direccion} onChange={e => setReceptor(r => ({ ...r, direccion: e.target.value }))}
                    placeholder="Ciudad"
                    className="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-blue-400" />
                </div>
              </div>
              ...
              <div className="border border-gray-100 rounded-xl overflow-hidden">
                  ...
                  <div key={i} className="grid grid-cols-12 gap-2 px-4 py-3 items-center border-b border-gray-50 last:border-0">
                    <div className="col-span-6">
                      <input value={item.descripcion} onChange={e => actualizarItem(i, 'descripcion', e.target.value)}
                        placeholder="Combustible regular, lubricante..."
                ...
                {totalEmision > 0 && (
                  <div className="grid grid-cols-12 px-4 py-3 bg-gray-50 border-t border-gray-100">
                    <div className="col-span-8 text-xs font-medium text-gray-600">Gran Total (con IVA incluido)</div>
